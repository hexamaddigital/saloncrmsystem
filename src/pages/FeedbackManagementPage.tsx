...
function StarRating({ rating }: { rating: number }) {
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map(n => (
        <Star key={n} className={`w-3.5 h-3.5 ${n <= rating ? 'text-amber-400 fill-amber-400' : 'text-gray-600'}`} />
      ))}
    </div>
  );
}
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="bg-white rounded-2xl border border-white/20 p-6 flex items-center gap-5">
            ...
            <div>
              <p className="text-xs text-gray-600 uppercase tracking-wide font-semibold">Average Rating</p>
              <p className="text-4xl font-extrabold text-gray-900 mt-1">{avg}</p>
              <p className="text-xs text-gray-600">{feedback.length} total reviews</p>
            </div>
          </div>
          <div className="bg-white rounded-2xl border border-white/20 p-6">
            <p className="text-xs text-gray-600 uppercase tracking-wide font-semibold mb-3">Rating Distribution</p>
        ...
        {showForm && (
          <div className="bg-white rounded-2xl border-2 border-teal-200 p-6 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wide mb-1">Rating *</label>
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex gap-1 p-1 bg-white/40 rounded-xl flex-wrap">
            <button onClick={() => setRatingFilter('all')}
              className={`px-3 py-1.5 rounded-lg text-sm font-semibold transition ${ratingFilter === 'all' ? 'bg-white/80 text-teal-700' : 'text-gray-600 hover:text-gray-700'}`}>All</button>
            {[5, 4, 3, 2, 1].map(r => (
              <button key={r} onClick={() => setRatingFilter(r)}
                className={`px-3 py-1.5 rounded-lg text-sm font-semibold transition flex items-center gap-1 ${ratingFilter === r ? 'bg-white text-amber-600' : 'text-gray-600 hover:text-gray-700'}`}>
                {r}<Star className="w-3 h-3 fill-current" />
              </button>
            ))}
          </div>
          <button onClick={fetchFeedback} className="p-2 hover:bg-white rounded-lg transition"><RefreshCw className="w-4 h-4 text-gray-600" /></button>
        </div>
              ...
              <div key={f.id} className="glass-subtle rounded-xl border border-white/20 p-4 flex gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    ...
                    <span className="text-xs text-gray-600">{f.clients?.phone}</span>
                    <StarRating rating={f.rating} />
                  </div>
                  {f.comment && <p className="text-sm text-gray-700 leading-relaxed">{f.comment}</p>}
                  <p className="text-xs text-gray-600 mt-1">{fmtDate(f.date)}</p>
                </div>
                <button onClick={() => deleteFeedback(f.id)} className="p-1.5 text-gray-600 hover:text-red-500 hover:bg-red-50 rounded-lg transition shrink-0 self-start">
                  <X className="w-3.5 h-3.5" />
                </button>
              </div>
            ))}
            {filtered.length === 0 && <div className="bg-white rounded-2xl border border-white/20 p-10 text-center text-gray-600 text-sm">No feedback found.</div>}
          </div>
        )}
      </main>
    </div>
