        ...
        {showForm && (
          <div className="bg-white rounded-2xl border-2 border-teal-200 p-6 shadow-sm space-y-4">
            ...
            <div className="flex gap-2">
              <button onClick={() => setShowForm(false)} disabled={saving} className="flex-1 py-2.5 border border-gray-300 text-gray-700 font-semibold rounded-xl hover:bg-gray-50 transition text-sm">Cancel</button>
              <button onClick={saveCoupon} disabled={saving} className="flex-1 py-2.5 bg-teal-600 hover:bg-teal-700 text-white font-bold rounded-xl transition text-sm flex items-center justify-center gap-2 disabled:bg-gray-400">
          ...
          <div className="flex gap-1 p-1 bg-gray-100 rounded-xl">
            {(['all', 'active', 'inactive'] as const).map(f => (
              <button key={f} onClick={() => setFilter(f)}
                className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition capitalize ${filter === f ? 'bg-white text-teal-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                {f}
              </button>
            ))}
          </div>
              ...
              return (
                <div key={c.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-3">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <span className="font-mono font-bold text-lg text-teal-700 bg-teal-50 px-3 py-0.5 rounded-lg">{c.code}</span>
                  ...
                  <div className="flex gap-2 pt-2 border-t border-gray-100">
                    ...
                    <button onClick={() => openEdit(c)} className="flex-1 flex items-center justify-center gap-1 py-1.5 text-xs font-semibold text-gray-600 hover:text-teal-700 hover:bg-teal-50 rounded-lg border border-gray-200 hover:border-teal-300 transition">
                      <Pencil className="w-3 h-3" /> Edit
                    </button>
                    <button onClick={() => deleteCoupon(c.id)} className="flex-1 flex items-center justify-center gap-1 py-1.5 text-xs font-semibold text-gray-600 hover:text-red-600 hover:bg-red-50 rounded-lg border border-gray-200 hover:border-red-300 transition">
