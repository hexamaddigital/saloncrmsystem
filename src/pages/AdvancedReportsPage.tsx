      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-40">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={downloadCSV} className="flex items-center gap-1.5 px-3 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-semibold rounded-xl transition">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            {(Object.keys(rangeLabels) as DateRange[]).map(r => (
              <button key={r} onClick={() => setDateRange(r)}
                className={`py-2 px-3 rounded-xl text-sm font-semibold border-2 transition ${dateRange === r ? 'bg-teal-600 text-white border-teal-600' : 'bg-white text-gray-600 border-gray-200 hover:border-teal-300'}`}>
                {rangeLabels[r]}
              </button>
            ))}
          </div>
          {dateRange === 'custom' && (
            <div className="flex gap-3 flex-wrap">
              <div>
                ...
                <input type="date" value={customFrom} onChange={e => setCustomFrom(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-teal-500 outline-none" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">To</label>
                <input type="date" value={customTo} onChange={e => setCustomTo(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-teal-500 outline-none" />
              </div>
            </div>
          )}
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Service Filter</label>
            <select value={serviceFilter} onChange={e => setServiceFilter(e.target.value)} className="w-full sm:w-64 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-teal-500 outline-none">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {summaries.map(s => (
                <div key={s.label} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                  <div className={`w-10 h-10 rounded-xl bg-gray-50 flex items-center justify-center ${s.color} mb-3`}>{s.icon}</div>
                    ...
                    <tbody>
                      {clientMems.map((m, i) => (
                        <tr key={i} className="border-b border-gray-50 hover:bg-gray-50">
                          ...
                          <td className="py-2.5 px-3"><span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${m.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>{m.status}</span></td>
            ...
            {couponUsage.length > 0 && (
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                  {couponUsage.map(c => (
                    <div key={c.code} className="bg-gray-50 rounded-xl border border-gray-200 p-3 text-center">
